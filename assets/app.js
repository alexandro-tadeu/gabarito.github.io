...
function loadPhoto(e) {
  ...
  img.onerror = () => {
    alert("Não consegui abrir essa imagem. Tente tirar uma nova foto em JPG/PNG ou usar outra imagem da galeria.");
    document.getElementById("photoStatus").textContent = "Erro ao abrir a imagem enviada.";
  };
...
function readOMRFromWarpedTable(warped, modeLabel) {
  ...
  const fillThreshold = parseFloat(document.getElementById("fillThreshold").value || "0.09");
  const diffThreshold = parseFloat(document.getElementById("diffThreshold").value || "0.012");
  ...
  let validationBeforeFallback = { ok: true, reason: "" };

  if (typeof validateFullTableGrid === "function") {
    validationBeforeFallback = validateFullTableGrid(warped, rawGrid);
  }

  // Versão tolerante:
  // Se a grade real não for detectada com segurança, o sistema NÃO bloqueia.
  // Ele usa uma grade geométrica calculada pelo tamanho da tabela corrigida.
  let grid = validationBeforeFallback.ok
    ? normalizeGridLinesForOMR(warped, rawGrid)
    : fallbackGridLines(w, h);
  ...
  if (!validationBeforeFallback.ok) {
    document.getElementById("photoStatus").textContent =
      "A grade não foi detectada perfeitamente, então usei a leitura de segurança. Confira as respostas na conferência rápida.";
  }
  ...
  const currentMsg = document.getElementById("photoStatus").textContent || "";
  const fallbackMsg = currentMsg.includes("leitura de segurança")
    ? " A grade automática falhou, mas a leitura de segurança foi aplicada."
    : "";

  document.getElementById("photoStatus").textContent =
    `Leitura concluída: ${lidas} de ${questionCount} respostas prováveis.${fallbackMsg} Confira antes de corrigir.`;
}
...
async function readByTableCorners() {
  try {
    ...
    if (manualCorners.length !== 4) {
      alert("A foto foi carregada, mas ainda faltam os 4 cantos da tabela. Toque nos 4 cantos externos da borda preta da tabela inteira e tente novamente.");
      return;
    }
